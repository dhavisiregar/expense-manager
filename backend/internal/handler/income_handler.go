...
func (h *IncomeHandler) List(w http.ResponseWriter, r *http.Request) {
	q := r.URL.Query()
	filter := domain.IncomeFilter{Page: 1, PageSize: 20}
	...
	response.JSONWithMeta(w, http.StatusOK, incomes, meta)
}
...
func (h *IncomeHandler) GetByID(w http.ResponseWriter, r *http.Request) {
	...
	income, err := h.svc.GetByID(r.Context(), id)
	if err != nil {
		response.Error(w, http.StatusNotFound, "income not found")
		return
	}
	response.JSON(w, http.StatusOK, income)
}
...
func (h *IncomeHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	input := domain.UpdateIncomeInput{
		Title:       req.Title,
		Amount:      req.Amount,
		Source:      req.Source,
		Description: req.Description,
	...
	income, err := h.svc.Update(r.Context(), id, input)
	if err != nil {
		response.Error(w, http.StatusBadRequest, err.Error())
		return
	}
