...
export function IncomeClient({
  ...
  const handleUpdate = async (input: CreateIncomeInput) => {
    ...
    setSubmitting(true);
    try {
      await updateIncome(editTarget.id, {
        title: input.title,
        amount: input.amount,
        source: input.source,
        date: input.date,
    ...
    <>
      <style>{`
        @media (max-width: 768px) {
          .income-page { padding: 20px 16px !important; }
          .inc-col-date { display: none !important; }
        }
      `}</style>

      <div className="income-page" style={{ padding: "32px" }}>
        <PageHeader
          title="Income"
        ...
        {/* Summary strip — side by side on all sizes */}
        {filtered.length > 0 && (
          <div
            style={{
              display: "grid",
